...
#include "gnss/parser/asensing/asensing_parser.h"

namespace apollo {
namespace drivers {
namespace gnss {

void AsensingParser::GetMessages(std::vector<Parser::Message>* messages) {
  std::string frame;
...
bool AsensingParser::ExtractFrame(std::string* frame) {
  if (data_ == nullptr) {
    return false;
  }
...
bool AsensingParser::ParseMessage(const std::string& frame) {
  if (!CheckCRC(frame)) {
...
bool AsensingParser::CheckCRC(const std::string& frame) {
  size_t l = frame.size() - huace::CRC_LENGTH;
  return crc32_block(frame.data(), l) ==
         *reinterpret_cast<uint32_t*>(frame.data() + l);
}

bool AsensingParser::HandleGPCHC(const std::vector<std::string>& items) {
...
void AsensingParser::FillMessages(std::vector<Parser::Message>* messages) {
  FillGnssBestpos();
  FillImu();
  FillHeading();
  FillIns();
