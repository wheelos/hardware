...
#include "gnss/parser/broadgnss/broadgnss_parser.h"

namespace apollo {
namespace drivers {
namespace gnss {

void HuaceParser::GetMessages(std::vector<Parser::Message>* messages) {
  std::string frame;
...
bool HuaceParser::ExtractFrame(std::string* frame) {
  if (data_ == nullptr) {
    return false;
  }
...
bool HuaceParser::ParseMessage(const std::string& frame) {
  if (!CheckCRC(frame)) {
...
bool HuaceParser::CheckCRC(const std::string& frame) {
  size_t l = frame.size() - huace::CRC_LENGTH;
  return crc32_block(frame.data(), l) ==
         *reinterpret_cast<uint32_t*>(frame.data() + l);
}

bool HuaceParser::HandleGPCHC(const std::vector<std::string>& items) {
...
bool HuaceParser::HandleGPCHCX(const std::vector<std::string>& items) {
  HandleGPCHC(items);

  gpchcx_->latitude_std = std::stod(items[24]);
  gpchcx_->longitude_std = std::stod(items[25]);
  gpchcx_->altitude_std = std::stod(items[26]);
  gpchcx_->ve_std = std::stod(items[27]);
  gpchcx_->vn_std = std::stod(items[28]);
  gpchcx_->vu_std = std::stod(items[29]);
  gpchcx_->roll_std = std::stod(items[30]);
  gpchcx_->pitch_std = std::stod(items[31]);
  gpchcx_->heading_std = std::stod(items[32]);
  gpchcx_->speed_heading = std::stod(items[34]);
  gpchcx_->speed_heading_std = std::stod(items[35]);
  gpchcx_->antenna_x = std::stod(items[36]);
  gpchcx_->antenna_y = std::stod(items[37]);
  gpchcx_->antenna_z = std::stod(items[38]);
  gpchcx_->rotation_angle_x = std::stod(items[39]);
  gpchcx_->rotation_angle_y = std::stod(items[40]);
  gpchcx_->rotation_angle_z = std::stod(items[41]);
  gpchcx_->direction_angle = std::stod(items[42]);
  gpchcx_->main_antenna_satellite_count = str_to_uint8(items[43]);
  gpchcx_->secondary_antenna_satellite_count = str_to_uint8(items[44]);
  str_to_char_array(items[45], gpchcx_->device_sn);
  return true;
}

void HuaceParser::FillMessages(std::vector<Parser::Message>* messages) {
  FillGnssBestpos();
  FillImu();
  FillHeading();
  FillIns();
...
void HuaceParser::FillGnssBestpos() {
