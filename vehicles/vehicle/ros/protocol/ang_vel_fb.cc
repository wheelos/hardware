...
#include "vehicles/vehicle/ros/protocol/ang_vel_fb.h"
...
void AngVelFb::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_ros()->mutable_ang_vel_fb()->set_angular_velocity_y(
      angular_velocity_y(bytes, length));
  chassis->mutable_ros()->mutable_ang_vel_fb()->set_angular_velocity_z(
      angular_velocity_z(bytes, length));
  chassis->mutable_ros()->mutable_ang_vel_fb()->set_battery_voltage(
