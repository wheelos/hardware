...
#include "vehicles/vehicle/ros/ros_controller.h"
...
Chassis RosController::chassis() {
  ...
  if (chassis_detail.ros().has_twist_fb() &&
      chassis_detail.ros().twist_fb().has_x_speed()) {
    chassis_.set_x_speed(
        static_cast<float>(chassis_detail.ros().twist_fb().x_speed()));
  }

  if (chassis_detail.ros().has_twist_fb() &&
      chassis_detail.ros().twist_fb().has_y_speed()) {
    chassis_.set_y_speed(
        static_cast<float>(chassis_detail.ros().twist_fb().y_speed()));
  }

  if (chassis_detail.ros().has_twist_fb() &&
      chassis_detail.ros().twist_fb().has_z_angle_speed()) {
    chassis_.set_z_angle_speed(
        static_cast<float>(chassis_detail.ros().twist_fb().z_angle_speed()));
  }

  if (chassis_detail.ros().has_acc_fb() &&
      chassis_detail.ros().acc_fb().has_acceleration_x()) {
    chassis_.set_acceleration_x(
        static_cast<float>(chassis_detail.ros().acc_fb().acceleration_x()));
  }

  if (chassis_detail.ros().has_acc_fb() &&
      chassis_detail.ros().acc_fb().has_acceleration_y()) {
    chassis_.set_acceleration_y(
        static_cast<float>(chassis_detail.ros().acc_fb().acceleration_y()));
  }

  if (chassis_detail.ros().has_acc_fb() &&
      chassis_detail.ros().acc_fb().has_acceleration_z()) {
    chassis_.set_acceleration_z(
        static_cast<float>(chassis_detail.ros().acc_fb().acceleration_z()));
  }

  if (chassis_detail.ros().has_acc_fb() &&
      chassis_detail.ros().acc_fb().has_angular_velocity_x()) {
    chassis_.set_angular_velocity_x(
        static_cast<float>(chassis_detail.ros().acc_fb().angular_velocity_x()));
  }

  if (chassis_detail.ros().has_ang_vel_fb() &&
      chassis_detail.ros().ang_vel_fb().has_angular_velocity_y()) {
    chassis_.set_angular_velocity_y(
        chassis_detail.ros().ang_vel_fb().angular_velocity_y());
  }

  if (chassis_detail.ros().has_ang_vel_fb() &&
      chassis_detail.ros().ang_vel_fb().has_angular_velocity_z()) {
    chassis_.set_angular_velocity_z(
        chassis_detail.ros().ang_vel_fb().angular_velocity_z());
  }

  if (chassis_detail.ros().has_battery_voltage() &&
      chassis_detail.ros().ang_vel_fb().has_battery_voltage()) {
    chassis_.set_battery_voltage(
        chassis_detail.ros().ang_vel_fb().battery_voltage());
  }
...
void RosController::SetBeam(const ControlCommand& command) {
...
void RosController::SetHorn(const ControlCommand& command) {
...
void RosController::SetTurningSignal(const ControlCommand& command) {
  // Set Turn Signal
