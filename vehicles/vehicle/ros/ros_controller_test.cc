...
class RosControllerTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    std::string canbus_conf_file =
        "vehicles/testdata/conf/devkit_canbus_conf_test.pb.txt";
...
TEST_F(RosControllerTest, Status) {
  controller_.Init(params_, &sender_, &msg_manager_);
...
TEST_F(RosControllerTest, UpdateDrivingMode) {
  controller_.Init(params_, &sender_, &msg_manager_);
  controller_.set_driving_mode(Chassis::COMPLETE_AUTO_DRIVE);
