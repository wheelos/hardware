...
#include "vehicles/vehicle/yuhesen_mkmini/yuhesen_controller.h"

#include "modules/canbus/common/canbus_gflags.h"
#include "modules/canbus/vehicle/vehicle_controller.h"
#include "modules/canbus/vehicle/yuhesen/devkit_message_manager.h"
...
ErrorCode YuhesenController::Init(
    const VehicleParameter& params,
    CanSender<::apollo::canbus::ChassisDetail>* const can_sender,
    MessageManager<::apollo::canbus::ChassisDetail>* const message_manager) {
  if (is_initialized_) {
    AINFO << "YuhesenController has already been initiated.";
  ...
  AINFO << "YuhesenController is initialized.";

  is_initialized_ = true;
  return ErrorCode::OK;
}

YuhesenController::~YuhesenController() {}

bool YuhesenController::Start() {
  if (!is_initialized_) {
    AERROR << "YuhesenController has NOT been initiated.";
    return false;
  }
...
void YuhesenController::Stop() {
  if (!is_initialized_) {
    AERROR << "YuhesenController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "YuhesenController stopped.";
  }
}

Chassis YuhesenController::chassis() {
  chassis_.Clear();
  ...
  // 5 wheel spd
  if (chassis_detail.yuhesen().has_wheelspeed_report_506()) {
    if (chassis_detail.yuhesen().wheelspeed_report_506().has_rr()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_rr(
          chassis_detail.yuhesen().wheelspeed_report_506().rr());
    }
    if (chassis_detail.yuhesen().wheelspeed_report_506().has_rl()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_rl(
          chassis_detail.yuhesen().wheelspeed_report_506().rl());
    }
    if (chassis_detail.yuhesen().wheelspeed_report_506().has_fr()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_fr(
          chassis_detail.yuhesen().wheelspeed_report_506().fr());
    }
    if (chassis_detail.yuhesen().wheelspeed_report_506().has_fl()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_fl(
          chassis_detail.yuhesen().wheelspeed_report_506().fl());
    }
  }
  // 6 speed_mps
  if (chassis_detail.yuhesen().has_vcu_report_505() &&
      chassis_detail.yuhesen().vcu_report_505().has_speed()) {
    chassis_.set_speed_mps(
        static_cast<float>(chassis_detail.yuhesen().vcu_report_505().speed()));
  } else {
  ...
  // 9 throttle
  if (chassis_detail.yuhesen().has_throttle_report_500() &&
      chassis_detail.yuhesen()
          .throttle_report_500()
          .has_throttle_pedal_actual()) {
    chassis_.set_throttle_percentage(
        static_cast<float>(chassis_detail.yuhesen()
                               .throttle_report_500()
  ...
  // 10 brake
  if (chassis_detail.yuhesen().has_brake_report_501() &&
      chassis_detail.yuhesen().brake_report_501().has_brake_pedal_actual()) {
    chassis_.set_brake_percentage(static_cast<float>(
        chassis_detail.yuhesen().brake_report_501().brake_pedal_actual()));
  } else {
    chassis_.set_brake_percentage(0);
  }
  // 23, previously 11 gear
  if (chassis_detail.yuhesen().has_gear_report_503() &&
      chassis_detail.yuhesen().gear_report_503().has_gear_actual()) {
    Chassis::GearPosition gear_pos = Chassis::GEAR_INVALID;

    if (chassis_detail.yuhesen().gear_report_503().gear_actual() ==
  ...
  // 12 steering
  if (chassis_detail.yuhesen().has_steering_report_502() &&
      chassis_detail.yuhesen().steering_report_502().has_steer_angle_actual()) {
    chassis_.set_steering_percentage(static_cast<float>(
        chassis_detail.yuhesen().steering_report_502().steer_angle_actual() *
        100.0 / vehicle_params_.max_steer_angle() * M_PI / 180));
  } else {
    chassis_.set_steering_percentage(0);
  }
  // 13 parking brake
  if (chassis_detail.yuhesen().has_park_report_504() &&
      chassis_detail.yuhesen().park_report_504().has_parking_actual()) {
    if (chassis_detail.yuhesen().park_report_504().parking_actual() ==
  ...
  // 14 battery soc
  if (chassis_detail.yuhesen().has_bms_report_512() &&
      chassis_detail.yuhesen().bms_report_512().has_battery_soc()) {
    chassis_.set_battery_soc_percentage(
        chassis_detail.yuhesen().bms_report_512().battery_soc());
  } else {
...
void YuhesenController::Emergency() {
  set_driving_mode(Chassis::EMERGENCY_MODE);
  ResetProtocol();
}

ErrorCode YuhesenController::EnableAutoMode() {
...
ErrorCode YuhesenController::DisableAutoMode() {
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode YuhesenController::EnableSteeringOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
  ...
  AFATAL << "SpeedOnlyMode is not supported in yuhesen!";
  return ErrorCode::CANBUS_ERROR;
}

ErrorCode YuhesenController::EnableSpeedOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
void YuhesenController::Gear(Chassis::GearPosition gear_position) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
void YuhesenController::Brake(double pedal) {}

void YuhesenController::Throttle(double pedal) {}

void YuhesenController::Acceleration(double acc) {}

// yuhesen default, -30 ~ 00, left:+, right:-
// need to be compatible with control module, so reverse
// steering with default angle speed, 25-250 (default:250)
// angle:-99.99~0.00~99.99, unit:, left:-, right:+
void YuhesenController::Steer(double angle) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
...
void YuhesenController::Steer(double angle, double angle_spd) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
...
void YuhesenController::SetEpbBreak(const ControlCommand& command) {
...
void YuhesenController::SetBeam(const ControlCommand& command) {
...
void YuhesenController::SetHorn(const ControlCommand& command) {
...
void YuhesenController::SetTurningSignal(const ControlCommand& command) {
  // Set Turn Signal do not support on yuhesen
}

void YuhesenController::ResetProtocol() {
  message_manager_->ResetSendMessages();
}

bool YuhesenController::CheckChassisError() {
  ChassisDetail chassis_detail;
  message_manager_->GetSensorData(&chassis_detail);
  if (!chassis_detail.has_devkit()) {
    AERROR_EVERY(100) << "ChassisDetail has no yuhesen vehicle info."
                      << chassis_detail.DebugString();
    return false;
  }

  yuhesen yuhesen = chassis_detail.yuhesen();

  // steer fault
  if (yuhesen.has_steering_report_502()) {
    if (Steering_report_502::STEER_FLT1_STEER_SYSTEM_HARDWARE_FAULT ==
        yuhesen.steering_report_502().steer_flt1()) {
      return true;
    }
  }
  // drive fault
  if (yuhesen.has_throttle_report_500()) {
    if (Throttle_report_500::THROTTLE_FLT1_DRIVE_SYSTEM_HARDWARE_FAULT ==
        yuhesen.throttle_report_500().throttle_flt1()) {
      return true;
    }
  }
  // brake fault
  if (yuhesen.has_brake_report_501()) {
    if (Brake_report_501::BRAKE_FLT1_BRAKE_SYSTEM_HARDWARE_FAULT ==
        yuhesen.brake_report_501().brake_flt1()) {
      return true;
    }
  }
...
void YuhesenController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (elapsed < default_period) {
      ...
      AERROR
          << "Too much time consumption in YuhesenController looping process:"
          << elapsed.count();
    }
  }
}

bool YuhesenController::CheckResponse(const int32_t flags, bool need_wait) {
...
void YuhesenController::set_chassis_error_mask(const int32_t mask) {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  chassis_error_mask_ = mask;
}

int32_t YuhesenController::chassis_error_mask() {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  return chassis_error_mask_;
}

Chassis::ErrorCode YuhesenController::chassis_error_code() {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  return chassis_error_code_;
}

void YuhesenController::set_chassis_error_code(
